    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <form method="GET" action="{{ route('santri.kartu-pembayaran') }}" class="flex gap-4 items-center">
                ...
                <noscript>
                    <button type="submit"
                        class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">Tampilkan</button>
                </noscript>
            </form>
        </div>
        ...
        @if ($kartuPembayaran)
            ...
            <div class="bg-white rounded-lg shadow p-8">
                ...
                <div class="text-center border-2 border-black p-4 mb-6">
                    <div class="flex justify-between items-center mb-4">
                        <div class="text-left">
                            <img src="/logo/pesantren-logo.png" alt="Logo" class="h-16 w-16 mb-2"
                    ...
                    <div class="grid grid-cols-2 gap-4 text-left text-sm">
                        ...
                        <div>
                            <p><strong>KELAS/TINGKAT:</strong> {{ $user->tingkatan }}/{{ $user->kelas ?? '-' }}
                                (MI/SMP/MTS/MA/SJ)</p>
                        </div>
                        <div>
                    ...
                    <table class="w-full border-collapse border border-black text-xs">
                        <thead>
                            <tr class="bg-purple-200">
                                ...
                                @foreach ($kategoriTagihan as $kategori)
                                    ...
                                    @endphp
                                    <th class="border border-black px-1 py-1 text-center font-bold"
                                        colspan="{{ $spanCount }}">
                        ...
                        <tbody>
                            ...
                            @foreach ($bulanNames as $index => $bulanName)
                                ...
                                <tr>
                                    <td class="border border-black px-2 py-1 font-bold bg-purple-100">
                                        {{ strtoupper($bulanName) }}</td>
                                    @foreach ($kategoriTagihan as $kategori)
                                        @php
                                            $sudahBayar =
                                                isset($pembayaranData[$bulanNumber][$kategori->id]['sudah_bayar']) &&
                                                $pembayaranData[$bulanNumber][$kategori->id]['sudah_bayar'];
                                            $jumlah = $pembayaranData[$bulanNumber][$kategori->id]['jumlah'] ?? '';
                                            $tanggalBayar =
                                                $pembayaranData[$bulanNumber][$kategori->id]['tanggal_bayar'] ?? '';
                                        @endphp
                                        ...
                                        @if ($kategori->nama == 'Imtihan')
                                            ...
                                            <td class="border border-black px-1 py-1 text-center text-xs">
                                                @if ($sudahBayar && $bulanNumber == 12)
                                                    {{-- Desember untuk Imtihan Awal --}}
                                                    <div class="text-xs">{{ number_format($jumlah) }}</div>
                                                    <div class="text-xs">
                                                        {{ $tanggalBayar ? \Carbon\Carbon::parse($tanggalBayar)->format('d.m') : '' }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="border border-black px-1 py-1 text-center">
                                                @if ($sudahBayar && $bulanNumber == 12)
                                                    <div
                                                        class="w-6 h-6 border border-black rounded-full mx-auto bg-blue-200">
                                                    </div>
                                                @endif
                                            </td>
                                            <!-- Imtihan Tsani -->
...
@endsection
<div class="bg-white rounded-lg shadow p-6">
    <h3 class="text-lg font-semibold text-green-700 border-b border-gray-200 pb-4 mb-6">Statistik Pembayaran
    </h3>
...
<div class="bg-white rounded-lg shadow p-6">
    <h3 class="text-lg font-semibold text-green-700 border-b border-gray-200 pb-4 mb-6">Cara Penggunaan Kartu
    </h3>

    <div class="prose max-w-none">
        <ol class="list-decimal list-inside space-y-3">
            <li>Kartu pembayaran hanya dapat digunakan oleh santri yang bersangkutan</li>
            <li>Nomor kartu digunakan untuk identifikasi saat melakukan pembayaran</li>
            <li>Pastikan kartu dalam status aktif untuk dapat melakukan transaksi</li>
            <li>Gunakan nomor kartu saat membuat tagihan atau melakukan pembayaran</li>
            <li>Jika kartu hilang atau rusak, segera hubungi bendahara untuk penggantian</li>
            <li>Simpan kartu dengan baik dan jangan berikan kepada orang lain</li>
        </ol>
    </div>
</div>
...
<div class="bg-white rounded-lg shadow p-12">
    <div class="text-center">
        ...
        <h3 class="text-xl font-semibold text-gray-800 mb-3">Kartu Pembayaran Belum Tersedia</h3>
        <p class="text-gray-600 mb-6 max-w-md mx-auto">
            Kartu pembayaran Anda belum dibuat oleh admin. Silakan hubungi admin atau bendahara untuk pembuatan
            kartu pembayaran.
        </p>
        <div class="space-y-3">
            ...
            <p class="text-sm text-gray-600">
                Admin: admin@pesantren.ac.id<br>
                Bendahara: bendahara@pesantren.ac.id
            </p>
        </div>
        <div class="mt-8">
            <a href="{{ route('santri.dashboard') }}"
