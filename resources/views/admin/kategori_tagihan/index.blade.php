...
@section('content')
    <div x-data="{ openModal: false, editMode: false, item: {} }">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Data Kategori Tagihan</h1>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($kategoriTagihan as $kategori)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <button
                                            @click="openModal = true; editMode = true; item = { id: {{ $kategori->id }}, nama: '{{ $kategori->nama }}' }"
                                        ...
                                        <form action="{{ route('admin.kategori-tagihan.destroy', $kategori->id) }}"
                                            method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="px-6 py-4 text-center text-gray-500">Belum ada data kategori
                                    tagihan.</td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="openModal" class="fixed inset-0 overflow-y-auto z-50" style="display: none;">
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <form
                        :action="editMode ? '{{ route('admin.kategori-tagihan') }}/' + item.id :
                            '{{ route('admin.kategori-tagihan.store') }}'"
                        method="POST">
                        @csrf
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-lg leading-6 font-medium text-gray-900"
                                x-text="editMode ? 'Edit Kategori Tagihan' : 'Tambah Kategori Tagihan'"></h3>
                            <div class="mt-4">
                                <label for="nama" class="block text-sm font-medium text-gray-700">Nama Kategori</label>
                                <input type="text" name="nama" id="nama" x-model="item.nama" required
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                    placeholder="Contoh: Bulanan, Tahunan, Uang Pangkal">
                            </div>
                        </div>
